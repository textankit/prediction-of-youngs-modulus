### This code first loads data.json and uses shear modulus (G_VRH) as the target. After data selection, the features are generated. 
### ML data including the target and features are then saved in formula_K.json and features.csv, respectively.
### After training, the ML model for shear modulus is saved as rf_shear_modulus.joblib.
...
for i in range(num_data):
    if(data[i]['elasticity']['K_VRH']==None or data[i]['elasticity']['G_VRH']==None):
        ...
        label_list.append(data[i]['elasticity']["G_VRH"])
...
import json
with open('formula_G.json', 'w') as outfile:
...
import joblib
joblib.dump(tree, 'rf_shear_modulus.joblib')
